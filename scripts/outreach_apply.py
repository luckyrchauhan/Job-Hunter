...
def _claude_draft_outreach(job, lead, outreach_type, first_name) -> str:
    ...
    try:
        ...
        prompt = f"""You are drafting a {outreach_type} LinkedIn/email outreach message for Lucky Chauhan.
...
def _claude_tailor_resume(job: dict) -> str:
    ...
    try:
        ...
        prompt = f"""You are tailoring Lucky Chauhan's resume for a specific job.
...
def _claude_cover_letter(job, lead, first_name, resume_path) -> str:
    ...
    try:
        ...
        prompt = f"""Write a cover letter for Lucky Chauhan applying to {company} for {title}.
